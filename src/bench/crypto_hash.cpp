...
static void COutPoint_equality_mismatch(benchmark::Bench& bench)
{
    FastRandomContext rng(true);
    constexpr size_t size = 1000;

    std::vector<COutPoint> outpoints1(size), outpoints2(size);
    for (size_t i = 0; i < size; ++i) {
        outpoints1[i] = {Txid::FromUint256(rng.rand256()), rng.rand32()};
        outpoints2[i] = {Txid::FromUint256(rng.rand256()), rng.rand32()};
        assert(outpoints1[i] != outpoints2[i]);
    }

    bench.batch(size).run([&] {
        bool result = false;
        for (size_t i = 0; i < size; ++i) {
            result ^= (outpoints1[i] == outpoints2[i]);
        }
        ankerl::nanobench::doNotOptimizeAway(result);
    });
}

static void COutPoint_equality_match(benchmark::Bench& bench)
{
    FastRandomContext rng(true);
    constexpr size_t size = 1000;
...
BENCHMARK(COutPoint_equality_mismatch, benchmark::PriorityLevel::HIGH);
BENCHMARK(COutPoint_equality_match, benchmark::PriorityLevel::HIGH);
