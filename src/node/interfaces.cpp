...
public:
    ...
    bool waitFeesChanged(MillisecondsDouble timeout, uint256 tip, CAmount fee_delta, CAmount fees_before) override
    {
        Assume(getTipHash());
        ...
        {
            while (!chainman().m_interrupt && std::chrono::steady_clock::now() < deadline) {
                std::this_thread::sleep_for(std::min(timeout, MillisecondsDouble(1000)));
                if (getTipHash().value() != tip) return false;
