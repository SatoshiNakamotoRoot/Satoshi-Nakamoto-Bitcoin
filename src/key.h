...
public:
     ...
     * allows for passing the KeyPair directly in the following manner:
     *
     *     reinterpret_cast<const secp256k1_keypair*>(keypair.data())
     *
     * Recall that secp256k1_keypair is an opaque datatype, so this method should only be used
     ...
     */
    const unsigned char* data() const { return m_keypair ? m_keypair->data() : nullptr; }
